...
export async function getStaticProps() {
  ...
  try {
    ...
    const reposRes = await fetch(
      `https://api.github.com/users/joashneves/repos?sort=updated&direction=desc`,
      { headers },
    );

    if (!userRes.ok || !reposRes.ok) {
      throw new Error(
        `Failed to fetch GitHub data: user ${userRes.status}, repos ${reposRes.status}`,
      );
    }
    ...
    console.error(
      "Failed to fetch GitHub data in getStaticProps:",
      error.message,
    );
